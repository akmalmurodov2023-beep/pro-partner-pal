...
type Client = { id: string; company_name: string; inn: string | null; bank_account: string | null; telegram_archive_link: string | null };
...
function ClientsPage() {
  ...
  const load = async () => {
    ...
    if (data) setRows(data as Client[]);
  };
  useEffect(() => { load(); }, []);
  ...
  return (
    <div>
      <PageHeader title={t("clients")} action={
        <Button onClick={() => { setEditing({ company_name: "", inn: "", bank_account: "", telegram_archive_link: "" }); setOpen(true); }}>
      ...
      <div className="border rounded-lg bg-card overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 && <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-8">{t("no_data")}</TableCell></TableRow>}
            {filtered.map(c => (
              <TableRow key={c.id}>
